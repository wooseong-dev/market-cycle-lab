...
templates.env.globals["fmt_months"] = fmt_months
...
def simulations_index(request: Request):
    return templates.TemplateResponse(
        "simulations_index.html",
        {
            "request": request,
            "settings": settings,
            "page_title": "투자 시뮬레이션",
            "page_description": "적립식 투자, 고점 매수, 하락장 생존, 목표 금액, 포트폴리오 버티기 난이도를 계산합니다.",
        },
    )
...
def simulation_dca(
    request: Request,
    symbol: str = Query("BTC-USD"),
    amount: float = Query(100_000),
    frequency: str = Query("monthly"),
    years: int = Query(3),
):
    symbol = clean_symbol(symbol)
    df, _ = get_history(symbol)
    result = simulate_dca(df, amount=amount, frequency=frequency, years=years)
    asset = build_asset(symbol)
    ...
    return templates.TemplateResponse(
        "simulation_dca.html",
        {
            ...
            "years": years,
            "result": result,
            "page_title": "적립식 투자 시뮬레이션",
            "page_description": "매주 또는 매월 일정 금액을 투자했을 때의 결과를 계산합니다.",
        },
    )
...
def simulation_peak(
    ...
    symbol = clean_symbol(symbol)
    df, _ = get_history(symbol)
    result = simulate_peak_buy(df, amount=amount, years=years)
    asset = build_asset(symbol)

    return templates.TemplateResponse(
        "simulation_peak.html",
        {
            "request": request,
            "settings": settings,
            "asset": asset,
            ...
            "years": years,
            "result": result,
            "page_title": "고점 매수 생존 시뮬레이션",
            "page_description": "최근 고점에 투자했다면 현재 얼마가 되었는지 계산합니다.",
        },
    )
...
def simulation_bear(
    ...
    symbol = clean_symbol(symbol)
    df, _ = get_history(symbol)
    result = simulate_bear_survival(df, years=years)
    asset = build_asset(symbol)

    return templates.TemplateResponse(
        "simulation_bear.html",
        {
            "request": request,
            "settings": settings,
            "asset": asset,
            ...
            "years": years,
            "result": result,
            "page_title": "하락장 생존 통계",
            "page_description": "과거 하락 구간의 깊이와 기간을 계산합니다.",
        },
    )
...
def simulation_target(
    request: Request,
    target: float = Query(100_000_000),
    initial: float = Query(5_000_000),
    monthly: float = Query(300_000),
    custom_rate: float = Query(12),
):
    result = simulate_target_goal(target=target, initial=initial, monthly=monthly, custom_rate=custom_rate)

    return templates.TemplateResponse(
        "simulation_target.html",
        {
            "request": request,
            "settings": settings,
            "target": target,
            ...
            "result": result,
            "page_title": "목표금액 도달 시뮬레이션",
            "page_description": "목표 금액에 도달하기까지 필요한 시간을 가정 수익률별로 계산합니다.",
        },
    )
...
def simulation_portfolio(
    request: Request,
    symbols: str = Query("BTC-USD,QQQ,GLD"),
    weights: str = Query("40,40,20"),
):
    ...
    for symbol, raw_weight, norm_weight in zip(symbol_list, weight_list, normalized):
        asset = build_asset(symbol)
        asset["portfolio_weight"] = raw_weight
        asset["portfolio_weight_norm"] = norm_weight
        assets.append(asset)
    ...
    if weighted_difficulty >= 75:
        label = "상당히 피곤한 포트폴리오"
    elif weighted_difficulty >= 55:
        label = "중간 이상의 변동성 포트폴리오"
    elif weighted_difficulty >= 35:
        label = "중립적 포트폴리오"
    else:
        label = "비교적 안정적인 포트폴리오"

    return templates.TemplateResponse(
        "simulation_portfolio.html",
        {
            "request": request,
            "settings": settings,
            "symbols": symbols,
            ...
            "label": label,
            "page_title": "포트폴리오 버티기 난이도",
            "page_description": "여러 자산의 비중을 입력해 포트폴리오의 버티기 난이도를 계산합니다.",
        },
    )
...
def framework_page(
    request: Request,
    symbol: str = Query("BTC-USD"),
    account_type: str = Query("core"),
    allocation_pct: float = Query(10),
    risk_tolerance: str = Query("medium"),
    valuation_view: str = Query("neutral"),
    macro_view: str = Query("neutral"),
    supply_view: str = Query("neutral"),
    exit_rule: str = Query("yes"),
):
    asset = build_asset(symbol)
    framework = build_framework_analysis(
        asset=asset,
